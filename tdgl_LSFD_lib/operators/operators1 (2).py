...
class LSFD_operators:
    def __init__(
        ...
        self.normal_vecs_length = 10**(-5) # use as weight for boundary conditions and Poisson equation,
        ...
        # === ЗАМЕНЯЕМ МАССИВЫ СОСЕДЕЙ НА GHOST-ВЕРСИИ ===
        ...
        self.mu_rhs_indices = self.nb_indices  # (N, K)
    ...
    def compute_W_matrix(self, matrix_type: str = 'psi',  # 'psi', 'psi_gamma', или 'mu'
        ...
        W_matrix = (np.exp( - nb_dist**2 / (self.domain_radii[:, np.newaxis])**2)).astype(self.dtype)#(1.0 / nb_dist).astype(self.dtype)
    ...
    def compute_delta_psi(self, psi: np.ndarray, A_applied: np.ndarray, s_applied: np.ndarray,
        ...
        if psi_ghost is None:
            psi_ghost = psi[self.boundary_indices]

        # Расширяем psi, чтобы индексы >= N корректно мапились на ghost значения
    ...
    def build_delta_matrix(self, A_applied) -> sp.csr_array:
        ...
        if self.Delta_matrix is not None and np.array_equal(A_applied, self._A_cache):
            # Быстрая проверка: если массивы идентичны → возвращаем кэш
        ...
        Delta_matrix = sp.csr_array(
            (values, (self.rows, self.cols)), shape=(len(self.edge_indices), self.n_sites + self.n_ghosts)
        )
    ...
    def compute_mu_rhs(self, mu_guess: np.ndarray, div_J: np.ndarray,
                       I_boundary: np.ndarray, mu_ghost: np.ndarray):
        """
        ...
        mu_ext = np.concatenate([ mu_ghost, mu_guess])

        # Основное заполнение: mu[neighbor_j]
        rhs = mu_ext[self.mu_rhs_indices]  # (N, K)
    ...
    def solve_poisson(self, div_J: np.ndarray, I_boundary: np.ndarray,
                      ...
                      mu_ghost: np.ndarray = None,
                      tolerance: float = 1e-5,
                      max_iterations: int = 200):
        """
        Решает уравнение Пуассона для μ итерационным методом.

        Args:
            div_J: (N,) — дивергенция тока
            I_boundary: (N_boundary,) — ток на границе
            mu_guess: (N,) — начальное приближение
            tolerance: Критерий сходимости
            max_iterations: Максимум итераций

        Returns:
            mu: (N,) — скалярный потенциал
            gradients: (N, 2) — градиент (∂xμ, ∂yμ)
        """
        ...
        for iteration in range(max_iterations):
            ...
            # 1. Вычисляем RHS
            rhs = self.compute_mu_rhs(mu, div_J, I_boundary, mu_ghost)  # (N, K)

            # 2. Применяем G_matrix: derivatives = G @ rhs
            # G_matrix_mu: (N, 15, K)
            # rhs: (N, K)
            # result: (N, 15)
            derivatives = np.einsum('nji,ni->nj', self.G_matrix_mu, rhs,
                                    optimize='optimal')

            # 3. Извлекаем μ и градиенты
            mu_new = derivatives[:, 0]  # μ (константа)
            gradients = derivatives[:, 1:3]  # (∂xμ, ∂yμ)

            # 4. Проверка сходимости
            error = np.sqrt(np.sum((mu_new - mu) ** 2) /
                            (np.sum(mu_new ** 2)))
