...
class Polygon:
    ...
    def make_mesh(
        ...
        """Returns a Mesh covering the Polygon.

        Args:
            min_points: Minimum number of vertices in the mesh.
            smooth: Number of Laplacian smoothing iterations.
            n_lsfd_neighbors: Number of neighbors for LSFD (saved in mesh).
        ...
        # 2. Создаём Mesh с глобальным параметром
        mesh = Mesh.from_triangulation(
            points,
            triangles,
            n_lsfd_neighbors=n_lsfd_neighbors,  # ← передаём
        )

        # 3. Сглаживание
        if smooth:
            mesh = mesh.smooth(iterations=smooth)

        logger.debug(
            f"Mesh created: {mesh.n_sites} sites, {mesh.n_elements} triangles, "
            f"LSFD_neighbors={mesh.n_lsfd_neighbors}"  # ← лог с параметром
        )
        return mesh
    ...
    def to_hdf5(self, h5_group: h5py.Group, compress: bool = False) -> None:
        ...
        h5_group.create_dataset("points", data=self.points, compression=compression)
